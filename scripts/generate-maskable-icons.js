...
// Updated turquoise blue colors
const PRIMARY_START = { r: 0x1D, g: 0xAD, b: 0xE4 };
const PRIMARY_END   = { r: 0x0E, g: 0x7F, b: 0xA8 };
...
function generateMaskablePNG(size) {
  const width = size;
  const height = size;

  // Create RGBA pixel data - solid background for maskable icon
  ...
  // Draw "K" letter in white (simpler version for maskable)
...
for (const size of sizes) {
  const png = generateMaskablePNG(size);
