...
// SSL config must match server.js — Scalingo/Clever Cloud/Neon all require SSL
...
async function runFolderMigrations(client) {
  ...
  console.log(`[migrate] Found ${files.length} migration files: ${JSON.stringify(files)}`);
  ...
  console.log(`[migrate] Already applied: ${JSON.stringify([...appliedNames])}`);
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.log(`[migrate] Complete: ${migrationName}`);
    } catch (err) {
      await client.query('ROLLBACK');
      throw new Error(`Migration failed (${migrationName}): ${err.message}`);
    }
  }
}
